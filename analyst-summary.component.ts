...
export class CreditAnalystSummaryComponent implements OnInit {
	...
	getAnalystList() {
		var analystList = this.creditService.getAnalystList().then((result: GetAnalystListResponse) => {
	...
	getCreditApplicationData() {
		var creditApplicationDetails = this.creditService.getCreditRequest(this.creditService.CreditRequestIdFromAnalystDashboard).then((result: GetCreditApplicationDetailResponse) => {
			...
			console.log("Failed to get Credit request Application data");
		})
	...
	public disableSave() {
		if (this.freezeform.dirty) {
			if (!this.selectedStatusAssigned) {
				this.isSave = false;
				this.statusErrorHighlight = true;
				return;
			}
			else if (this.selectedStatusAssigned && (this.selectedStatusAssigned.value === Credit_WorkFlow_Status.APPR || this.selectedStatusAssigned.value === Credit_WorkFlow_Status.DENI)) {
				this.isBillToIncorrect = this.model && this.model.updatedBillToNumber && this.isBillToIncorrect ? this.isBillToIncorrect : false;

				if (this.model && this.model.updatedBillToNumber) {
					this.statusErrorHighlight = false;
					this.isSave = !this.isBillToIncorrect;
				}
				else if (this.model && !this.model.updatedBillToNumber) {
					this.isBillToIncorrect = true;
					this.statusErrorHighlight = false;
					this.isSave = false;
				}
				else {
					this.isBillToIncorrect = this.model && this.model.updatedBillToNumber ? this.isBillToIncorrect : false;
					this.statusErrorHighlight = true;
					this.isSave = true;
				}
			}
			else if (this.selectedStatusAssigned) {
				this.isBillToIncorrect = this.model && this.model.updatedBillToNumber ? this.isBillToIncorrect : false;
				this.statusErrorHighlight = false;
				this.isSave = true;
			}
			...
			if (!this.selectedAssignTo) {
				this.isSave = false;
				this.assignToErrorHighlight = true;
				return;
			}
			else {
				this.assignToErrorHighlight = false;
				this.isSave = true;
			}
			let analystBillTo = this.freezeform.controls['analyst-bill-to'];
			let analystShipper = this.freezeform.controls['analyst-shipper-number'];

			if (analystBillTo && analystBillTo.dirty && this.isBillToIncorrect) {
				this.isSave = false;
				return;
			}

			if (analystShipper && analystShipper.dirty && this.isShipperIncorrect) {
				this.isSave = false;
				return;
			}
		}
		else if (!this.freezeform.dirty) {
			this.isSave = false;
			return;
		}
		else {
	...
	public saveFreezeSection() {
		if (this.model.id) {
			...
			this.creditService.saveCreditApplicationDetail(request).then((result: any) => {
	...
	private preProcessCreditBillToSection(creditDetailClone: CreditApplicationDetail) {
		...
		if (creditDetailClone.updatedBillToNumber &&
			...
			this.customerService.getCustomerDetail(requestBillingInfo).then((result: GetCustomerDetailResponse) => {
	...
	private preProcessCreditShipperSection(creditDetailClone: CreditApplicationDetail) {
		...
		if (creditDetailClone.updatedShipperNumber && (creditDetailClone.shipperNumber !== creditDetailClone.updatedShipperNumber)) {
			...
			this.customerService.getCustomerDetail(requestShipperInfo).then((result: GetCustomerDetailResponse) => {
				if (result.errors.hasErrors) {
					this.isShipperIncorrect = true;
